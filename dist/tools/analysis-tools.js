 ...
 */
export function registerAnalysisTools(server, ccxtServer) {
    // 거래 성과 요약 분석 도구
    server.tool("analyzeTradingPerformance", "Analyze trading performance for a configured account", {
        accountName: z
            .string()
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error analyzing trading performance for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
    ...
    // 승률 및 수익률 분석 도구
    server.tool("calculateWinRate", "Calculate win rate and profit metrics for a configured account", {
        accountName: z
            .string()
            .describe("Account name defined in the configuration file (e.g., 'bybit_main')"),
        symbol: z
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error calculating win rate for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
    ...
    // 최대 연속 손실/이익 분석 도구
    server.tool("analyzeConsecutiveProfitLoss", "Analyze consecutive winning and losing trades", {
        accountName: z
            .string()
            .describe("Account name defined in the configuration file (e.g., 'bybit_main')"),
        symbol: z
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error analyzing consecutive trades for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
    ...
    // 월간/일간 수익률 분석 도구
    server.tool("analyzePeriodicReturns", "Analyze daily and monthly returns for a configured account", {
        accountName: z
            .string()
            .describe("Account name defined in the configuration file (e.g., 'bybit_main')"),
        symbol: z
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error analyzing periodic returns for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
 ...
 */
function analyzeTradeData(trades, period) {
 ...
 */
function calculateWinRateAndProfitMetrics(trades) {
    if (!trades || trades.length === 0) {
        return {
            totalTrades: 0,
            message: "No trades found for the specified period.",
 ...
 */
function analyzeConsecutiveTrades(trades) {
    if (!trades || trades.length === 0) {
        return {
            totalTrades: 0,
            message: "No trades found for the specified period.",
 ...
 * 일간/주간/월간 수익률을 계산합니다.
 */
function calculatePeriodicReturns(trades, interval) {
    if (!trades || trades.length === 0) {
        return {
            totalTrades: 0,
            message: "No trades found for the specified period.",
