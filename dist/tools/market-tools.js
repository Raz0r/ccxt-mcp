 ...
 */
export function registerMarketTools(server, ccxtServer) {
    // 시장 정보 조회 도구
    server.tool("fetchMarkets", "Fetch markets from a cryptocurrency exchange", {
    ...
    // 티커 정보 조회 도구
    server.tool("fetchTicker", "Fetch ticker information for a symbol on an exchange", {
        exchangeId: z.string().describe("Exchange ID (e.g., 'binance', 'coinbase')"),
    ...
    // 모든 티커 정보 조회 도구
    server.tool("fetchTickers", "Fetch all tickers from an exchange", {
        exchangeId: z.string().describe("Exchange ID (e.g., 'binance', 'coinbase')"),
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error fetching tickers: ${error.message}`
                    }
                ],
                isError: true
            };
    ...
    // 주문장 정보 조회 도구
    server.tool("fetchOrderBook", "Fetch order book for a symbol on an exchange", {
        exchangeId: z.string().describe("Exchange ID (e.g., 'binance', 'coinbase')"),
        symbol: z.string().describe("Trading symbol (e.g., 'BTC/USDT')"),
    ...
    // 거래 내역 조회 도구
    server.tool("fetchTrades", "Fetch recent trades for a symbol on an exchange", {
        exchangeId: z.string().describe("Exchange ID (e.g., 'binance', 'coinbase')"),
        symbol: z.string().describe("Trading symbol (e.g., 'BTC/USDT')"),
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error fetching trades: ${error.message}`
                    }
                ],
                isError: true
            };
    ...
    server.tool("fetchOHLCV", "Fetch OHLCV candlestick data for a symbol on an exchange", {
        exchangeId: z.string().describe("Exchange ID (e.g., 'binance', 'coinbase')"),
        symbol: z.string().describe("Trading symbol (e.g., 'BTC/USDT')"),
