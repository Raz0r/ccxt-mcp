 ...
 */
export function registerOrderTools(server, ccxtServer) {
    // 주문 생성 도구
    server.tool("createOrder", "Create a new order using a configured account", {
        accountName: z
            .string()
            .describe("Account name defined in the configuration file (e.g., 'bybit_main')"),
    ...
    }, async ({ accountName, symbol, type, side, amount, price, params, }) => {
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error creating order for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
    ...
    // 주문 취소 도구
    server.tool("cancelOrder", "Cancel an existing order using a configured account", {
        accountName: z
            .string()
            .describe("Account name defined in the configuration file (e.g., 'bybit_main')"),
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error canceling order for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
    ...
    // 주문 조회 도구
    server.tool("fetchOrder", "Fetch information about a specific order using a configured account", {
        accountName: z
            .string()
            .describe("Account name defined in the configuration file (e.g., 'bybit_main')"),
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error fetching order for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
    ...
    // 열린 주문 조회 도구
    server.tool("fetchOpenOrders", "Fetch all open orders using a configured account", {
        accountName: z
            .string()
            .describe("Account name defined in the configuration file (e.g., 'bybit_main')"),
        symbol: z
        ...
        try {
            ...
            const openOrders = await exchange.fetchOpenOrders(symbol, since, limit, params);
            return {
                content: [
                    {
                        type: "text",
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error fetching open orders for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
    ...
    // 마감된 주문 조회 도구
    server.tool("fetchClosedOrders", "Fetch all closed orders using a configured account", {
        accountName: z
            .string()
            .describe("Account name defined in the configuration file (e.g., 'bybit_main')"),
        symbol: z
        ...
        try {
            ...
            const closedOrders = await exchange.fetchClosedOrders(symbol, since, limit, params);
            return {
                content: [
                    {
                        type: "text",
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error fetching closed orders for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
