 ...
 */
export function registerAccountTools(server, ccxtServer) {
    // 계정 목록 조회 도구
    server.tool("listAccounts", "List all configured account names", {}, async () => {
...
Your config should look like:
{
  "accounts": [
    ...
    // 계정 잔액 조회 도구
    server.tool("fetchBalance", "Fetch account balance for a configured account", {
        accountName: z
            .string()
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error fetching balance for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
    ...
    // 입금 내역 조회 도구
    server.tool("fetchDeposits", "Fetch deposit history for a configured account", {
        accountName: z
            .string()
            .describe("Account name defined in the configuration file (e.g., 'bybit_main')"),
        code: z
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error fetching deposits for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
    ...
    // 출금 내역 조회 도구
    server.tool("fetchWithdrawals", "Fetch withdrawal history for a configured account", {
        accountName: z
            .string()
            .describe("Account name defined in the configuration file (e.g., 'bybit_main')"),
        code: z
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error fetching withdrawals for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
    ...
    // 내 거래 내역 조회 도구
    server.tool("fetchMyTrades", "Fetch personal trade history for a configured account", {
        accountName: z
            .string()
            .describe("Account name defined in the configuration file (e.g., 'bybit_main')"),
        symbol: z
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error fetching personal trades for account '${accountName}': ${error.message}`,
                    },
                ],
                isError: true,
            };
