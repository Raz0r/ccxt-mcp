 ...
 */
export function registerExchangeResources(server, ccxtServer) {
    // 거래소 목록 리소스
    server.resource("exchanges", "exchanges://list", async (uri) => {
    ...
    // 특정 거래소 정보 리소스
    server.resource("exchange-info", new ResourceTemplate("exchange://info/{exchangeId}", {
        list: async () => {
            // 모든 거래소의 기본 정보 반환
        ...
        complete: {
            exchangeId: async (value) => {
                // 거래소 ID 자동완성 제공
                return ccxt.exchanges.filter(exchange => exchange.toLowerCase().includes(value.toLowerCase())).slice(0, 10);
            }
        }
    }), async (uri, variables) => {
