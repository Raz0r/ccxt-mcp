 ...
 */
export function registerPriceResources(server, ccxtServer) {
    // 특정 거래소의 특정 심볼에 대한 티커 정보 리소스
    server.resource("ticker", new ResourceTemplate("ticker://exchange/{exchangeId}/{symbol}", {
        list: undefined,
        complete: {
    ...
    // 특정 거래소의 모든 티커 정보
    server.resource("tickers", new ResourceTemplate("tickers://exchange/{exchangeId}", {
        list: undefined,
        complete: {
            exchangeId: async (value) => {
                return ccxt.exchanges
