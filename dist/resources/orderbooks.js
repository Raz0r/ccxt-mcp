 ...
 */
export function registerOrderBookResources(server, ccxtServer) {
    // 특정 거래소의 특정 심볼에 대한 주문장 정보 리소스
    server.resource("orderbook", new ResourceTemplate("orderbook://exchange/{exchangeId}/{symbol}/{limit?}", {
        list: undefined,
        complete: {
            exchangeId: async (value) => {
                return ccxt.exchanges
                    .filter((exchange) => exchange.toLowerCase().includes(value.toLowerCase()))
                    .slice(0, 10);
            },
        ...
        try {
            // 공개 인스턴스 사용
            const exchange = ccxtServer.getPublicExchangeInstance(exchangeId);
            const orderbook = await exchange.fetchOrderBook(symbol, limitNum);
            return {
                contents: [
                    {
                        uri: uri.href,
    ...
    server.resource("l2orderbook", new ResourceTemplate("l2orderbook://exchange/{exchangeId}/{symbol}", {
        list: undefined,
        complete: {
            exchangeId: async (value) => {
                return ccxt.exchanges
