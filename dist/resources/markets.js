 ...
 */
export function registerMarketResources(server, ccxtServer) {
    // 특정 거래소의 시장 정보 리소스
    server.resource("markets", new ResourceTemplate("markets://exchange/{exchangeId}", {
        list: async () => {
            // 모든 거래소의 기본 정보 반환
    ...
    server.resource("market", new ResourceTemplate("market://exchange/{exchangeId}/{symbol}", {
        list: undefined,
        complete: {
            exchangeId: async (value) => {
                return ccxt.exchanges
