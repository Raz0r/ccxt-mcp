 ...
 */
import { McpServer, } from "@modelcontextprotocol/sdk/server/mcp.js"; // Restore original path
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js"; // Restore original path
import ccxt from "ccxt"; // Import Exchange type
import fs from "fs";
...
export class CcxtMcpServer {
    ...
    async loadAccountsFromConfig() {
        try {
            ...
            try {
                ...
                throw new Error(`Invalid JSON format in config file: ${jsonError.message}`);
            }
            let accounts = [];
            ...
            // 외부 설정 파일에 계정 정보가 없으면 Claude Desktop 설정 구조 확인
                ...
                if (!mcpConfig || !Array.isArray(mcpConfig.accounts)) {
                    console.error(`[ERROR] Neither 'accounts' array nor 'mcpServers.ccxt-mcp.accounts' found in config`);
            ...
            const loadedAccountNames = new Set();
            for (const account of accounts) {
                ...
                if (!account.name ||
                    ...
                    !account.apiKey ||
                    !account.secret) {
                    console.warn(`Skipping account due to missing fields: ${JSON.stringify(account)}`);
                    continue;
                }
                // 중복 계정 이름 검사
                ...
                if (!ccxt.exchanges.includes(account.exchangeId)) {
                    console.error(`Unsupported exchange ID '${account.exchangeId}' for account '${account.name}'. Supported exchanges: ${ccxt.exchanges.join(", ")}`);
                    continue;
                }
                try {
                    ...
                    const exchangeInstance = new ccxt[account.exchangeId](exchangeOptions);
                    ...
                    if (account.defaultType) {
                        ...
                        if (!supportedTypes[account.defaultType]) {
                            console.error(`${account.exchangeId} does not support ${account.defaultType} trading. ` +
            ...
            console.error(`Failed to load or parse configuration file ${this.configPath}:`, error);
        }
    } // loadAccountsFromConfig 메소드 닫는 괄호
    /**
     * 선물 주문을 위한 거래소별 파라미터 반환
     */
    getFutureParams(exchangeId) {
        return ({
     ...
     * 선물 주문 실행 (자동 재시도 포함)
     */
    async executeFutureOrder(accountName, orderParams) {
        const exchange = this.getExchangeInstance(accountName);
        const futureParams = this.getFutureParams(exchange.id);
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                return await exchange.createOrder(orderParams.symbol, orderParams.type, orderParams.side, orderParams.amount, orderParams.price, {
                    ...(orderParams.params || {}),
                    ...futureParams,
                });
            }
     ...
     */
    getExchangeInstance(accountName) {
        const instance = this.exchangeInstances[accountName];
        if (!instance) {
            console.error(`No pre-loaded exchange instance found for account name: ${accountName}`);
            // Consider listing available account names: Object.keys(this.exchangeInstances).join(', ')
            throw new Error(`Account configuration not found or failed to load for: ${accountName}`);
        }
        return instance;
    }
    /**
    ...
    getPublicExchangeInstance(exchangeId, marketType = "spot") {
        const instanceKey = `${exchangeId}-${marketType}`;
        if (!this.publicExchangeInstances[instanceKey]) {
            if (!ccxt.exchanges.includes(exchangeId)) {
                console.error(`Exchange ID '${exchangeId}' not found in ccxt.exchanges for public instance.`);
            ...
            try {
                ...
                this.publicExchangeInstances[instanceKey] = new ccxt[exchangeId](exchangeOptions);
            }
            catch (error) {
                console.error(`Failed to create public CCXT instance for ${exchangeId} (${marketType}):`, error);
